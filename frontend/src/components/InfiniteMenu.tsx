...
void main() {
    ...
    // Removed geometry stretching on rotation for cleaner, non-jittery appearance
...
class ArcballControl {
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    if (this.isPointerDown) {
      const INTENSITY = 0.15 * timeScale; // Silkier drag follow
      ...
      if (this.snapTargetDirection) {
        const SNAPPING_INTENSITY = 0.08; // Much smoother snap when letting go
    ...
    const RA_INTENSITY = 0.35 * timeScale; // Heavier inertia easing for absolute smoothness
...
class InfiniteGridMenu {
  ...
  #deltaFrames = 0;
  #frames = 0;
  #animationFrameId = null; // Storing animation request ID correctly
  ...
  #render() {
    ...
    gl.clearColor(0, 0, 0, 0); // Transparent background to layer it
  ...
  #onControlUpdate(deltaTime) {
    ...
    if (!this.control.isPointerDown) {
      ...
      if (this.lastNearestIndex !== nearestVertexIndex) {
        this.onActiveItemChange(itemIndex);
        this.lastNearestIndex = nearestVertexIndex;
      }
...
export interface InfiniteMenuProps {
  ...
  scale?: number;
  onItemAction?: (item: InfiniteMenuItem) => void;
}

export default function InfiniteMenu({ items = [], scale = 1.0, onItemAction }: InfiniteMenuProps) {
  ...
  return (
    <div className="infinite-menu-container">
      ...
      {activeItem && (
        <div className="absolute inset-x-0 bottom-12 flex flex-col items-center justify-center pointer-events-none z-10">
          ...
          <div onClick={handleButtonClick} className={`action-button mt-4 pointer-events-auto ${isMoving ? 'inactive' : 'active'}`}>
