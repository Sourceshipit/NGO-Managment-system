  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="card p-6 flex flex-col md:flex-row items-start md:items-center gap-6">
        <div className="w-24 h-24 bg-blue-600 border border-brand-border flex items-center justify-center text-white text-4xl font-black font-mono">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-3 mt-3">
            <span className="text-xs px-2 py-1 bg-blue-100 border border-brand-border text-black font-bold font-mono">VOLUNTEER</span>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          <div className="card p-6">
            <h3 className="text-xl font-black uppercase tracking-widest text-black border-b border-brand-border pb-4 mb-6">Personal Info</h3>
            <div className="space-y-5">
              <div>
                <label className="text-sm font-bold font-mono text-black uppercase block mb-2">Full Name</label>
                <input value={name} onChange={e => setName(e.target.value)} className="w-full h-12 px-4 bg-white border border-brand-border font-mono text-black focus:outline-none focus:ring-0 focus:border-blue-500 rounded-none border-shadow-sm" />
              </div>
              <div>
                <label className="text-sm font-bold font-mono text-black uppercase block mb-2">Email</label>
                <input value={user?.email} readOnly className="w-full h-12 px-4 bg-slate-100 border border-brand-border font-mono text-slate-500 cursor-not-allowed rounded-none" />
              </div>
              <div>
                ...
                <textarea value={bio} onChange={e => setBio(e.target.value.slice(0, 300))} rows={4} className="w-full px-4 py-3 bg-white border border-brand-border font-mono text-black focus:outline-none focus:ring-0 focus:border-blue-500 resize-none rounded-none border-shadow-sm" />
              </div>
            </div>
            ...
            <div className="mt-8">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {skills.map(s => (
                  <span key={s} className="flex items-center gap-1.5 px-3 py-1 bg-blue-100 border border-brand-border text-black font-bold font-mono text-xs">
              ...
              <div className="flex gap-2 mb-4">
                <input value={newSkill} onChange={e => setNewSkill(e.target.value)} onKeyDown={e => e.key === 'Enter' && addSkill(newSkill)}
                  className="flex-1 h-12 px-4 bg-white border border-brand-border font-mono text-sm focus:outline-none focus:ring-0 focus:border-blue-500 rounded-none border-shadow-sm uppercase" placeholder="ADD SKILL..." />
                <button onClick={() => addSkill(newSkill)} className="h-12 px-4 bg-blue-600 border border-brand-border text-white hover:bg-blue-700 hover:-translate-y-0.5 hover:shadow-sm transition-all rounded-none"><Plus className="w-5 h-5" /></button>
              </div>
              <div className="flex flex-wrap gap-2">
                {suggestedSkills.filter(s => !skills.includes(s)).map(s => (
                  <button key={s} onClick={() => addSkill(s)} className="text-xs font-semibold px-2 py-1 bg-slate-100 border border-brand-border text-slate-600 hover:bg-blue-100 hover:text-black transition-colors rounded-none">{s}</button>
                ))}
              </div>
            </div>
            <button onClick={saveProfile} disabled={saving}
              className="w-full mt-8 py-3 bg-blue-600 text-white font-black font-mono uppercase tracking-widest border border-brand-border hover:bg-blue-700 hover:-translate-y-1 hover:shadow-md transition-all disabled:opacity-60 disabled:hover:translate-y-0 disabled:hover:shadow-none flex items-center justify-center gap-2 rounded-none">
          ...
          <div className="card p-6">
            ...
            {showPwd && (
              <div className="mt-6 space-y-4 pt-6 border-t border-brand-border">
                <input type="password" value={curPwd} onChange={e => setCurPwd(e.target.value)} placeholder="CURRENT PASSWORD" className="w-full h-12 px-4 bg-white border border-brand-border font-mono text-sm focus:outline-none focus:ring-0 focus:border-blue-500 rounded-none placeholder-slate-400 uppercase border-shadow-sm" />
                <input type="password" value={newPwd} onChange={e => setNewPwd(e.target.value)} placeholder="NEW PASSWORD" className="w-full h-12 px-4 bg-white border border-brand-border font-mono text-sm focus:outline-none focus:ring-0 focus:border-blue-500 rounded-none placeholder-slate-400 uppercase border-shadow-sm" />
                <input type="password" value={confirmPwd} onChange={e => setConfirmPwd(e.target.value)} placeholder="CONFIRM NEW PASSWORD" className="w-full h-12 px-4 bg-white border border-brand-border font-mono text-sm focus:outline-none focus:ring-0 focus:border-blue-500 rounded-none placeholder-slate-400 uppercase border-shadow-sm" />
                <button onClick={changePassword} disabled={pwdSaving}
                  className="w-full py-3 bg-black text-white font-black font-mono uppercase tracking-widest border border-brand-border hover:bg-slate-800 hover:-translate-y-1 hover:shadow-[4px_4px_0px_#2563eb] transition-all disabled:opacity-60 disabled:hover:translate-y-0 disabled:hover:shadow-none rounded-none mt-2">
        ...
        <div className="card p-6 h-fit bg-slate-50">
          <h3 className="text-xl font-black uppercase tracking-widest text-black border-b border-brand-border pb-4 mb-6">My Stats</h3>
          {stats && (
            <>
              ...
              <div className="mb-8 p-5 bg-white border border-brand-border">
                ...
                <div className="w-full h-4 bg-slate-100 border border-brand-border overflow-hidden relative">
                  <div className="absolute top-0 left-0 h-full bg-blue-600 border-r border-brand-border" style={{width: `${progress}%`}}></div>
              ...
              <h3 className="text-xl font-black uppercase tracking-widest text-black border-b border-brand-border pb-4 mb-6 mt-8">Achievements</h3>
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                {achievements.map(a => {
                  const unlocked = a.check(stats);
                  return (
                    <div key={a.name} className={`text-center p-4 border border-brand-border bg-white transition-all ${unlocked ? 'hover:-translate-y-1 hover:shadow-md' : 'opacity-40 grayscale'} flex flex-col items-center justify-center`}>
