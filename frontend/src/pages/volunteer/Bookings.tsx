...
export default function VolunteerBookings() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex gap-1 border-2 border-black bg-slate-100 p-1 mb-6 w-fit">
      ...
      {list.length === 0 ? (
        <div className="text-center py-16 border-2 border-black bg-white"><Calendar className="w-16 h-16 text-black mx-auto mb-4" /><p className="font-mono text-slate-500 uppercase">No {tab} bookings</p></div>
      ) : (
        <div className="space-y-4">
          {list.map(b => (
            <div key={b.id} className="card bg-white p-4 flex items-start gap-4">
              <div className={`border-2 border-black px-3 py-2 text-center min-w-[60px] bg-white`}>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 font-bold uppercase tracking-widest border-2 border-black ${
              ...
              <div className="flex flex-col gap-2">
                {tab === 'upcoming' && (
                  <>
                    <button onClick={() => generateICS(b)} className="px-3 py-1.5 text-[10px] font-bold tracking-widest uppercase border-2 border-black bg-white text-black hover:bg-black hover:text-white transition flex items-center justify-center gap-1">
                      <Download className="w-3 h-3" /> Calendar
                    </button>
                    <button onClick={() => setConfirm(b)} className="px-3 py-1.5 text-[10px] font-bold tracking-widest uppercase border-2 border-black bg-white text-red-500 hover:bg-red-500 hover:text-white transition flex items-center justify-center gap-1">
                      <X className="w-3 h-3" /> Cancel
                    </button>
                  </>
                )}
                {tab === 'past' && (
                  <button onClick={() => navigate('/volunteer/hours')} className="px-3 py-1.5 text-[10px] font-bold tracking-widest uppercase border-2 border-black bg-blue-500 text-black hover:bg-black hover:text-white transition">Log Hours</button>
                )}
                {tab === 'cancelled' && (
                  <button onClick={() => rebook(b)} className="px-3 py-1.5 text-[10px] font-bold tracking-widest uppercase border-2 border-black bg-white text-blue-600 hover:bg-blue-600 hover:text-white transition flex items-center justify-center gap-1">
      ...
      {confirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setConfirm(null)}>
          <div className="card bg-white p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <h3 className="text-xl font-black text-black uppercase tracking-widest mb-2 border-b-2 border-black pb-2">Cancel Booking?</h3>
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirm(null)} className="flex-1 py-2 border-2 border-black bg-white text-xs font-bold uppercase tracking-widest hover:bg-slate-100">Keep It</button>
              <button onClick={() => cancelBooking(confirm)} className="flex-1 py-2 border-2 border-black bg-red-500 text-white text-xs font-bold uppercase tracking-widest hover:bg-red-600 disabled:opacity-50">Yes, Cancel</button>
            </div>
          </div>
        </div>
      )}
