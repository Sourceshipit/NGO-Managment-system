...
export default function StaffDonors() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {tab === 'list' && (
        <div className="space-y-6">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} className="w-full max-w-sm h-12 pl-10 pr-4 bg-white border border-brand-border font-mono text-sm shadow-[4px_4px_0_0_rgba(0,0,0,1)] focus:outline-none focus:bg-emerald-50" placeholder="SEARCH DONORS..." />
          </div>
            ...
            <table className="w-full text-sm font-mono">
              ...
              <tbody>
                {filteredDonors.map(d => (
                  <tr key={d.id} className="border-b border-brand-border last:border-b-0 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3"><span className={`text-xs px-2 py-0.5 border border-brand-border shadow-[2px_2px_0_0_rgba(0,0,0,1)] font-bold uppercase ${d.is_verified?'bg-green-400 text-black':'bg-amber-400 text-black'}`}>{d.is_verified?'Verified':'Pending'}</span></td>
                    <td className="px-4 py-3 flex gap-2">
                      <button onClick={() => viewDonorDonations(d)} className="p-2 bg-blue-300 border border-brand-border shadow-[2px_2px_0_0_rgba(0,0,0,1)] hover:translate-y-[2px] transition-all text-black"><Eye className="w-4 h-4" /></button>
                      <button onClick={() => setEditDonor(d)} className="p-2 bg-amber-300 border border-brand-border shadow-[2px_2px_0_0_rgba(0,0,0,1)] hover:translate-y-[2px] transition-all text-black"><Edit className="w-4 h-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {tab === 'record' && (
        <div className="bg-white border border-brand-border rounded-xl p-6 shadow-[8px_8px_0_0_rgba(0,0,0,1)] max-w-lg mb-6">
          <h3 className="text-2xl font-black uppercase tracking-tight text-black mb-6 border-b border-brand-border pb-4">Record New Donation</h3>
          <div className="space-y-6 font-mono">
            <div>
              <label className="text-sm font-bold uppercase text-black mb-2 block">Donor</label>
              <select value={form.donor_id} onChange={e => setForm({...form, donor_id: parseInt(e.target.value)})} className="w-full h-12 px-4 border border-brand-border shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-sm focus:outline-none focus:bg-emerald-50">
                <option value={0}>SELECT DONOR...</option>
                {donors.map(d => <option key={d.id} value={d.id}>{d.full_name.toUpperCase()}</option>)}
              </select>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="text-sm font-bold uppercase text-black mb-2 block">Amount (₹)</label><input type="number" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} className="w-full h-12 px-4 border border-brand-border shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-sm focus:outline-none focus:bg-emerald-50" placeholder="50000" /></div>
              <div><label className="text-sm font-bold uppercase text-black mb-2 block">Project</label><select value={form.project} onChange={e => setForm({...form, project: e.target.value})} className="w-full h-12 px-4 border border-brand-border shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-sm focus:outline-none focus:bg-emerald-50"><option>Education</option><option>Healthcare</option><option>Livelihood</option><option>Environment</option></select></div>
            </div>
            <div>
              <label className="text-sm font-bold uppercase text-black mb-2 block">Payment Mode</label>
              <div className="flex gap-2">
                {['UPI','Bank','Cash','Cheque'].map(m => (
                  <button key={m} onClick={() => setForm({...form, payment_mode: m})} className={`flex-1 py-3 border border-brand-border text-sm font-bold uppercase transition-all ${form.payment_mode===m?'bg-emerald-400 shadow-[2px_2px_0_0_rgba(0,0,0,1)] text-black translate-y-[-2px]':'bg-white text-black hover:bg-emerald-50 shadow-[4px_4px_0_0_rgba(0,0,0,1)]'}`}>{m}</button>
                ))}
              </div>
            </div>
            <div><label className="text-sm font-bold uppercase text-black mb-2 block">Notes</label><textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} rows={3} className="w-full px-4 py-3 border border-brand-border shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-sm focus:outline-none focus:bg-emerald-50 resize-none" /></div>
            <button onClick={recordDonation} disabled={submitting} className="w-full py-4 mt-6 bg-emerald-400 text-black border border-brand-border font-black uppercase text-sm shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_rgba(0,0,0,1)] transition-all disabled:opacity-50 disabled:translate-y-[2px] disabled:shadow-[2px_2px_0_0_rgba(0,0,0,1)] flex items-center justify-center gap-2">
      ...
      {tab === 'reports' && (
        <div className="grid grid-cols-3 gap-6">
          <div className="bg-white border border-brand-border p-5 shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-center">
      ...
      {viewDonor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm" onClick={() => setViewDonor(null)}>
          <div className="bg-white border border-brand-border rounded-xl p-6 w-full max-w-lg shadow-[8px_8px_0_0_rgba(0,0,0,1)] rounded-none max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6 border-b border-brand-border pb-4">
              ...
              <button onClick={() => setViewDonor(null)} className="p-2 border border-brand-border hover:bg-emerald-400 shadow-[2px_2px_0_0_rgba(0,0,0,1)] transition-colors"><X className="w-5 h-5 text-black" /></button>
            </div>
              ...
              <table className="w-full text-sm font-mono border border-brand-border shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
                ...
                <tbody>{donorDonations.map(d => (
                  <tr key={d.id} className="border-b border-brand-border last:border-b-0 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3"><button onClick={() => openCertificate(d.id)} className="p-2 bg-blue-300 border border-brand-border shadow-[2px_2px_0_0_rgba(0,0,0,1)] hover:translate-y-[2px] transition-all text-black"><Download className="w-4 h-4" /></button></td>
                  </tr>
                ))}</tbody>
              </table>
            )}
      ...
      {editDonor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm" onClick={() => setEditDonor(null)}>
          <div className="bg-white border border-brand-border rounded-xl p-6 w-full max-w-sm shadow-[8px_8px_0_0_rgba(0,0,0,1)] rounded-none" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6 border-b border-brand-border pb-4">
              <h3 className="text-2xl font-black uppercase tracking-tight text-black">EDIT DONOR</h3>
              <button onClick={() => setEditDonor(null)} className="p-2 border border-brand-border hover:bg-emerald-400 shadow-[2px_2px_0_0_rgba(0,0,0,1)] transition-colors"><X className="w-5 h-5 text-black" /></button>
            </div>
            <div className="space-y-4 font-mono">
              <input defaultValue={editDonor.full_name} id="edit-name" className="w-full h-12 px-4 border border-brand-border shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-sm focus:outline-none focus:bg-emerald-50" placeholder="NAME *" />
              <input defaultValue={editDonor.pan_number || ''} id="edit-pan" className="w-full h-12 px-4 border border-brand-border shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-sm focus:outline-none focus:bg-emerald-50 uppercase" placeholder="PAN" />
              ...
              }} className="w-full mt-6 py-4 bg-emerald-400 text-black border border-brand-border font-black uppercase text-sm shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_rgba(0,0,0,1)] transition-all">SAVE CHANGES</button>
            </div>
          </div>
        </div>
      )}
