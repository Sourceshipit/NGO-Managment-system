    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex gap-4 mb-6 flex-wrap">
        {([['volunteer','Volunteer',Users],['children','Children',Baby],['donation','Donations',Heart],['compliance','Compliance',Shield]] as const).map(([key, label, Icon]) => (
          <button key={key} onClick={() => setTab(key as any)} className={`font-mono px-4 py-2 text-sm font-bold uppercase transition-all flex items-center gap-2 border-2 border-black ${tab===key?'bg-emerald-400 shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-black translate-y-[-2px]':'bg-white shadow-[2px_2px_0_0_rgba(0,0,0,1)] text-black hover:bg-emerald-50 hover:translate-y-[-2px] hover:shadow-[4px_4px_0_0_rgba(0,0,0,1)]'}`}>
      ...
      {tab === 'volunteer' && (
        <div className="space-y-6">
          <div className="grid grid-cols-4 gap-6">
            ...
            ].map(s => <div key={s.l} className="bg-white border-2 border-black p-5 shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-center"><p className="text-4xl font-black text-black font-mono">{s.v}</p><p className="text-xs mt-2 font-bold uppercase text-slate-500 tracking-wider">{s.l}</p></div>)}
          </div>
          <div className="bg-white border-2 border-black p-5 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
            ...
            <table className="w-full text-sm font-mono"><thead><tr className="border-b-2 border-black text-slate-500 uppercase"><th className="text-left py-2 font-bold">Name</th><th className="text-left py-2 font-bold">Hours</th><th className="text-left py-2 font-bold">Skills</th></tr></thead>
          ...
          <button onClick={() => exportPDF('Volunteer Report', `<h1>Volunteer Report</h1><div class="stat"><h3>${vols.length}</h3><p>Total Volunteers</p></div><div class="stat"><h3>${vols.reduce((s:number,v:any)=>s+v.total_hours,0).toFixed(0)}h</h3><p>Total Hours</p></div>`)}
            className="px-6 py-3 bg-emerald-400 text-black border-2 border-black font-black uppercase text-sm shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_rgba(0,0,0,1)] transition-all flex items-center justify-center gap-2"><Download className="w-5 h-5" /> EXPORT REPORT</button>
        </div>
      )}
      ...
      {tab === 'children' && (
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-6">
            {[{l:'TOTAL ENROLLED',v:children.length},{l:'ACTIVE',v:children.filter((c:any)=>c.is_active).length},{l:'PROGRAMS',v:new Set(children.map((c:any)=>c.program)).size}].map(s => <div key={s.l} className="bg-white border-2 border-black p-5 shadow-[4px_4px_0_0_rgba(0,0,0,1)] text-center"><p className="text-4xl font-black text-black font-mono">{s.v}</p><p className="text-xs mt-2 font-bold uppercase text-slate-500 tracking-wider">{s.l}</p></div>)}
          </div>
          <div className="bg-white border-2 border-black p-5 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
            ...
            <div className="space-y-4 font-mono">
              {Object.entries(children.reduce((acc:any,c:any) => ({...acc,[c.program]:(acc[c.program]||0)+1}),{})).map(([p,c]) => (
                <div key={p} className="flex items-center gap-4"><span className="w-40 text-sm font-bold uppercase">{p}</span><div className="flex-1 h-8 bg-slate-100 border-2 border-black overflow-hidden relative"><div className="h-full bg-emerald-400 border-r-2 border-black" style={{width:`${((c as number)/children.length)*100}%`}}></div></div><span className="text-sm font-black w-8 text-right">{c as number}</span></div>
              ))}
            </div>
          </div>
      ...
      {tab === 'donation' && (
        <div className="space-y-6">
          ...
          <div className="bg-white border-2 border-black p-5 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
            ...
            <div className="space-y-4 font-mono">
              {Object.entries(donations.reduce((acc:any,d:any)=>({...acc,[d.project]:(acc[d.project]||0)+d.amount}),{})).map(([p,a]) => (
                <div key={p} className="flex items-center gap-4"><span className="w-40 text-sm font-bold uppercase">{p}</span><div className="flex-1 h-8 bg-slate-100 border-2 border-black overflow-hidden relative"><div className="h-full bg-pink-400 border-r-2 border-black" style={{width:`${(a as number)/donations.reduce((s:number,d:any)=>s+d.amount,0)*100}%`}}></div></div><span className="text-sm font-black text-right w-32">₹{(a as number).toLocaleString()}</span></div>
              ))}
            </div>
          </div>
          <button onClick={() => {
          ...
          }} className="px-6 py-3 bg-pink-400 text-black border-2 border-black font-black uppercase text-sm shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:translate-y-[2px] hover:shadow-[2px_2px_0_0_rgba(0,0,0,1)] transition-all flex items-center justify-center gap-2"><Download className="w-5 h-5" /> DOWNLOAD CSV</button>
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className="bg-white border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] overflow-hidden">
            ...
            <tbody>{compliance.map((c:any)=>{const d=c.next_deadline?Math.ceil((new Date(c.next_deadline).getTime()-Date.now())/86400000):null;return(
              <tr key={c.id} className="border-b-2 border-black last:border-b-0 hover:bg-slate-50 transition-colors"><td className="px-4 py-3 font-bold uppercase">{c.policy_name}</td><td className="px-4 py-3"><span className={`text-xs px-2 py-0.5 border-2 border-black shadow-[2px_2px_0_0_rgba(0,0,0,1)] font-bold uppercase ${c.status==='ACTIVE'?'bg-green-400 text-black':c.status==='DUE_SOON'?'bg-amber-400 text-black':'bg-red-400 text-black'}`}>{c.status}</span></td><td className="px-4 py-3 font-bold uppercase text-slate-600">{c.next_deadline?new Date(c.next_deadline).toLocaleDateString():'—'}</td><td className={`px-4 py-3 font-black ${d&&d<=30?'text-red-600':'text-black'}`}>{d!==null?d:'—'}</td></tr>
            )})}</tbody></table>
          </div>
        </div>
      )}
