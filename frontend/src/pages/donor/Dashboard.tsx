  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div className="card bg-pink-600 p-8 text-white relative overflow-hidden">
        <div className="absolute right-0 top-0 w-64 h-64 border border-brand-border rounded-xl border-r-0 border-t-0 rounded-bl-full translate-x-1/4 -translate-y-1/4"></div>
        ...
        <button onClick={() => setShowDonate(true)} className="mt-4 px-6 py-3 bg-white text-pink-600 border border-brand-border font-mono text-sm font-black uppercase tracking-widest hover:bg-black hover:text-white transition-all shadow-[4px_4px_0_#000] hover:shadow-[2px_2px_0_#000] hover:translate-x-[2px] hover:translate-y-[2px] relative flex items-center gap-2">
          <Heart size={16} /> DONATE NOW
        </button>
        <div className="mt-8 relative border-t border-brand-border pt-6"><p className="text-5xl font-black font-mono">₹{total.toLocaleString()}</p><p className="font-mono text-black font-bold uppercase mt-2">Your lifetime contributions</p></div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          <div key={c.l} className="card p-5">
            <div className={`w-12 h-12 border border-brand-border ${c.color} flex items-center justify-center mb-4`}><c.icon className="w-6 h-6 text-black" /></div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="card p-6">
          <h3 className="text-xl font-black uppercase tracking-widest text-black mb-6 border-b border-brand-border pb-4">Where Money Goes</h3>
            ...
            {Object.entries(byProject).map(([p,a]) => (
              <div key={p}>
                ...
                <div className="w-full h-4 bg-slate-100 border border-brand-border overflow-hidden relative">
                  <div className="absolute top-0 left-0 h-full bg-pink-600 border-r border-brand-border" style={{width:`${((a as number)/total)*100}%`}}></div>
        ...
        <div className="card p-6">
          ...
          {recent.length === 0 ? <p className="text-slate-500 font-mono text-center py-8">No donations yet</p> : (
            <div className="space-y-3">
              {recent.map(d => (
                <div key={d.id} className="flex items-center justify-between p-3 bg-white border border-brand-border hover:-translate-y-0.5 hover:shadow-sm transition-all">
                  <div>
                  ...
                  <span className="text-[10px] font-black font-mono px-2 py-1 bg-green-100 border border-brand-border text-black uppercase">{d.payment_mode}</span>
                </div>
              ))}
            </div>
          )}
