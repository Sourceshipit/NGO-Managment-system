...
const Employees: React.FC = () => {
  ...
  const getCellClass = (cell: any) => {
    ...
    if (cell.dateStr === todayStr) base += "ring-2 ring-brand-primary ring-offset-1 ";
    ...
    if (cell.isWeekend) return base + "bg-slate-100 text-slate-400 border-slate-200 border-2 shadow-sm";
    
    return base + "bg-white text-slate-600 hover:bg-slate-50 cursor-pointer border border-brand-border shadow-sm hover:shadow-md transition-all";
  };
  ...
  return (
    <div className="page-enter">
      <div className="flex justify-between items-end mb-6">
        <h1 className="page-title !mb-0 text-3xl font-black text-black ">Employee Tracking</h1>
      ...
      <div className="grid grid-cols-4 gap-5 mb-8">
        <div className="bg-white border border-brand-border shadow-card p-4 flex flex-col justify-center transition-all">
          <div className="text-[10px] uppercase tracking-wider text-slate-600 font-bold font-mono">Total Staff</div>
          <div className="text-2xl font-semibold text-slate-800">{employees.length}</div>
        </div>
        <div className="bg-white border border-brand-border shadow-card p-4 flex flex-col justify-center transition-all">
          <div className="text-[10px] uppercase tracking-wider text-slate-600 font-bold font-mono">Present Today</div>
          <div className="text-2xl font-semibold text-green-600">{employees.length - leavesToday}</div>
        </div>
        <div className="bg-white border border-brand-border shadow-card p-4 flex flex-col justify-center transition-all">
          <div className="text-[10px] uppercase tracking-wider text-slate-600 font-bold font-mono">On Leave</div>
          <div className="text-2xl font-semibold text-amber-500">{leavesToday}</div>
        </div>
        <div className="bg-white border border-brand-border shadow-card p-4 flex flex-col justify-center transition-all">
          <div className="text-[10px] uppercase tracking-wider text-slate-600 font-bold font-mono">Monthly Payroll</div>
          <div className="text-2xl font-semibold text-brand-primary">₹{payroll ? formatIndianCurrency(payroll.total_payroll) : '0'}</div>
        </div>
      </div>

      <div className="grid grid-cols-5 gap-6 mb-6">
        <div className="col-span-3 card !p-0 overflow-hidden flex flex-col h-[500px]">
          <div className="p-5 border-b border-brand-border bg-slate-50 z-10">
              ...
              columns={[
                { key: 'full_name', header: 'Employee Name', render: (row) => (
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-brand-primary-light text-brand-primary flex items-center justify-center font-bold text-xs shrink-0">{row.full_name[0]}</div>
                ...
                { key: 'action', header: 'Action', render: (row) => (
                  <button 
                    ...
                    className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${selectedEmployee?.id === row.id ? 'bg-brand-primary text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                  >
                    View
                  </button>
                )}
        ...
        <div className="col-span-2 card bg-slate-50">
          <h2 className="font-semibold text-slate-800 mb-6 border-b border-brand-border pb-2">Attendance Calendar</h2>
          {!selectedEmployee ? (
            <div className="h-64 flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-300 bg-white">
              ...
              <p className="font-semibold text-xs">Select an employee</p>
            </div>
          ) : (
            <div>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="font-semibold text-brand-primary">{format(viewMonth, 'MMMM yyyy')}</div>
      ...
      <div className="card !p-0 overflow-hidden mb-6">
        <div className="p-5 border-b border-brand-border bg-slate-50 flex justify-between items-center">
          ...
          <div className="flex bg-white border border-brand-border p-1 shadow-sm">
            {['All', 'Pending', 'Approved', 'Rejected'].map(tab => (
              <button 
                key={tab} 
                className={`text-[10px] px-3 py-1 font-semibold transition-all ${leaveTab === tab ? 'bg-black text-white' : 'text-slate-600 hover:bg-slate-100'}`}
      ...
      {isAdmin && (
        <div className="card bg-brand-primary-light border-brand-primary">
          <div className="grid grid-cols-3 gap-6">
            <div className="col-span-1 border-r border-brand-border pr-6 flex flex-col justify-center">
              <h2 className="text-slate-500 font-bold mb-1 tracking-widest uppercase text-[10px] font-mono">Monthly Payroll Summary</h2>
              <div className="text-4xl font-semibold text-brand-primary mb-2">₹{payroll ? formatIndianCurrency(payroll.total_payroll) : '0'}</div>
              <p className="text-xs text-slate-500 font-semibold">{employees.length} active employees processed</p>
            </div>
            <div className="col-span-1 pr-6 flex flex-col justify-center">
              <h3 className="font-semibold text-slate-700 mb-3 text-sm uppercase border-b border-brand-border pb-1 inline-block">Department Breakdown</h3>
              <div className="space-y-3 font-mono">
                {payroll?.breakdown.map((b: any, i: number) => (
                  <div key={i} className="flex justify-between text-xs font-bold uppercase border-b border-brand-primary/30 pb-1">
            ...
            <div className="col-span-1 flex flex-col justify-center items-center px-4">
              <button className="btn-secondary w-full py-4 text-brand-primary border-brand-primary/30 border bg-brand-primary-light hover:bg-brand-primary-light mb-3">
        ...
        <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="ADD EMPLOYEE" size="lg">
          <div className="grid grid-cols-2 gap-4 font-mono">
            ...
            <div className="col-span-2 border-t border-brand-border pt-4 mt-2">
              <h4 className="text-sm font-semibold bg-black text-white inline-block px-2 py-1 mb-3">Documents Uploaded</h4>
              <div className="flex gap-6">
                {['Aadhar', 'PAN', 'Offer Letter', 'Bank Details'].map(doc => (
                  <label key={doc} className="flex items-center gap-2 text-xs font-bold uppercase text-slate-700 cursor-pointer">
                    <input type="checkbox" checked={formData.documents.includes(doc)} onChange={() => toggleDoc(doc)} className="w-4 h-4 border border-brand-border" />
                    {doc}
                  </label>
                ))}
              </div>
            ...
            <div className="col-span-2 pt-4 flex justify-end gap-3 border-t border-brand-border mt-2">
              <button className="border border-brand-border px-4 py-2 font-bold uppercase hover:bg-slate-100" onClick={() => setIsModalOpen(false)}>Cancel</button>
