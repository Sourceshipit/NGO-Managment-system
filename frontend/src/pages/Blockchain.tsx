  ...
  return (
    <div className="page-enter">
      ...
      <div className="card mb-6 overflow-hidden bg-slate-50 border-black border-2 shadow-[6px_6px_0px_#000] pt-6 relative">
        ...
        <h2 className="font-mono font-black text-xl text-black mb-6 px-4  bg-yellow-300 inline-block py-1 border-y-2 border-black border-r-2 shadow-sm -ml-6 pl-8">Transaction Chain Visualization</h2>
        <div className="overflow-x-auto pb-6 pt-2 border-y-2 border-black bg-white px-6 custom-scrollbar">
          <div className="flex gap-6 items-center min-w-max py-4">
            ...
            {page === 1 && (!typeFilter || typeFilter === 'All') && (
              <>
                <div className="bg-black text-white p-5 w-60 flex-shrink-0 shadow-lg relative border border-brand-border transition-transform hover:-translate-y-1">
                  <div className="absolute -top-3 -right-3 text-white bg-black border border-brand-border rounded-full p-1 shadow-sm ring-2 ring-brand-primary/30"><ShieldCheck size={20}/></div>
                  <h3 className="font-mono font-black text-orange-400 mb-1 text-2xl tracking-tighter">GENESIS</h3>
                  <p className="text-[10px] text-slate-400 font-semibold border-b border-slate-700 pb-2 uppercase tracking-widest mb-3">Block #0</p>
            ...
            {[...logs].reverse().slice(0, 8).map((log, idx, arr) => (
              <React.Fragment key={log.id}>
                <div className={`bg-white border-2 p-5 w-60 flex-shrink-0 transition-transform hover:-translate-y-1 ${idx === arr.length - 1 ? 'border-brand-primary shadow-lg' : 'border-black shadow-md'}`}>
                  <div className="flex justify-between items-start mb-2 border-b border-brand-border pb-2">
                    <h3 className="font-mono font-black text-xl text-black">#{log.id}</h3>
                    <span className={`text-[10px] uppercase font-semibold px-2 py-1 border border-black ${log.record_type === 'DONATION' ? 'bg-brand-primary-light text-orange-800 shadow-sm ring-2 ring-brand-primary/30' : log.record_type === 'COMPLIANCE' ? 'bg-green-100 text-green-800 shadow-[2px_2px_0px_#22c55e]' : 'bg-blue-100 text-blue-800 shadow-[2px_2px_0px_#3b82f6]'}`}>
                      {log.record_type}
                    </span>
                  </div>
      ...
      <div className="card !p-0 border-black border-2 shadow-[8px_8px_0px_#000]">
        <DataTable 
          ...
          columns={[
            ...
            { key: 'type', header: 'TYPE', render: (row) => <span className={`text-[10px] uppercase font-semibold px-2 py-1 border border-black ${row.record_type === 'DONATION' ? 'bg-brand-primary-light text-orange-800 shadow-sm ring-2 ring-brand-primary/30' : row.record_type === 'COMPLIANCE' ? 'bg-green-100 text-green-800 shadow-[2px_2px_0px_#22c55e]' : 'bg-blue-100 text-blue-800 shadow-[2px_2px_0px_#3b82f6]'}`}>{row.record_type}</span> },
            ...
            { key: 'hash', header: 'TRANSACTION HASH', render: (row) => (
              <div className="flex items-center gap-2">
                <span className="font-semibold text-[10px] text-slate-700 bg-slate-100 border-2 border-slate-300 px-2 py-1">{row.tx_hash.slice(0, 10)}...{row.tx_hash.slice(-6)}</span>
                <button onClick={() => copyToClipboard(row.tx_hash)} className="text-black border border-brand-border shadow-sm hover:translate-x-[1px] hover:-translate-y-[1px] hover:shadow-[3px_3px_0px_#000] bg-white p-1 transition-all"><Copy size={14}/></button>
              </div>
            )},
        ...
        />
        <div className="p-4 border-t border-brand-border flex flex-col sm:flex-row justify-between items-center font-mono text-xs uppercase font-bold tracking-widest bg-slate-50">
